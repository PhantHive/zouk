...
module.exports = async (client, member) => {
  WDB.findOne({ server_id: member.guild.id }, async (err, data) => {
    ...
    // general ipsa server
    const generalPath = `./assets/img/welcome/${
      themes[parseInt(data.theme)]["name"]
    }`;
    let rand_back = [];
    ...
    if (
      member.guild.id === "502931781012684818" ||
      ...
      .then((channel) => {
        channel.send({ files: [attachment] }).then((r) => {
          ...
          let rulesChannel = member.guild.channels.cache.find(
            (ch) => ch.name.includes("règlement") || ch.name.includes("règles")
          );
          let globalMsg;
          ...
          if (member.guild.id === "717344084695580672") {
            member
              ...
              .then((channel) => {
                channel.send(
                  `Salut, bienvenue sur le discord  **${member.guild.name}** 😉\n` +
                    "\n" +
