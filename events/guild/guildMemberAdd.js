...
const applyText = (canvas, text, fontSize = null, maxPercentWidth) => {
  ...
  do {
    // Assign the font to the context and decrement it so it can be measured
    // again
    ctx.font = `italic bold ${fontSize -= 2}px Tahoma`;
    // Compare pixel width of the text to the canvas minus the approximate
    // avatar size
...
module.exports = async (client, member) => {
  WDB.findOne({server_id : member.guild.id}, async (err, data) => {
    if (!data)
    ...
    // general ipsa server
    const generalPath =
        `./assets/img/welcome/${themes[parseInt(data.theme)]["name"]}`;
    let rand_back = []
    readdirSync(generalPath)
        .forEach(file => { rand_back.push(`${generalPath}/${file}`); })
    ...
    if ((member.guild.id === '502931781012684818') ||
        (member.guild.id === '819986031809134643')) {
    ...
    // right top arc
    ctx.quadraticCurveTo(0.95 * canvas.width, 0.1 * canvas.height, canvas.width,
                         0.15 * canvas.height);
    // line to bottom right
    ctx.lineTo(canvas.width, 0.85 * canvas.height);
    // right bottom arc
    ctx.quadraticCurveTo(0.95 * canvas.width, 0.90 * canvas.height,
                         canvas.width, canvas.height);
    // line to bottom left
    ctx.lineTo(0, canvas.height);
    // left bottom arc
    ctx.quadraticCurveTo(0.05 * canvas.width, 0.90 * canvas.height, 0,
                         0.85 * canvas.height);
    // line to top left
    ...
    // draw same shape inside the canvas starting from the top left corner at
    // 0.05*canvas.width, 0.1*canvas.height
    ctx.beginPath();
    ctx.moveTo(0.05 * canvas.width, 0.1 * canvas.height);
    ctx.lineTo(0.95 * canvas.width, 0.1 * canvas.height);
    // right top arc
    ctx.quadraticCurveTo(0.94 * canvas.width, 0.14 * canvas.height,
                         0.95 * canvas.width, 0.15 * canvas.height);
    // line to bottom right
    ctx.lineTo(0.95 * canvas.width, 0.85 * canvas.height);
    // right bottom arc
    ctx.quadraticCurveTo(0.94 * canvas.width, 0.86 * canvas.height,
                         0.95 * canvas.width, 0.90 * canvas.height);
    // line to bottom left
    ctx.lineTo(0.05 * canvas.width, 0.90 * canvas.height);
    // left bottom arc
    ctx.quadraticCurveTo(0.06 * canvas.width, 0.86 * canvas.height,
                         0.05 * canvas.width, 0.85 * canvas.height);
    // line to top left
    ctx.lineTo(0.05 * canvas.width, 0.15 * canvas.height);
    // left top arc
    ctx.quadraticCurveTo(0.06 * canvas.width, 0.14 * canvas.height,
                         0.05 * canvas.width, 0.1 * canvas.height);
    ...
    // write "BOARDING PASS" in bottom between the bottom line and the bottom of
    // the canva
    let font = applyText(canvas, "BOARDING PASS", 42);
    ctx.font = font[0];
    ctx.fillStyle = guildColor;
    ctx.fillText("BOARDING PASS", 0.5 * canvas.width - (font[1] / 2),
                 0.98 * canvas.height);
    // stroke the text
    ctx.lineWidth = 0.5;
    ctx.strokeStyle = "#000000";
    ctx.strokeText("BOARDING PASS", 0.5 * canvas.width - (font[1] / 2),
                   0.98 * canvas.height);
    ...
    ctx.font = font[0];
    ctx.fillStyle = guildColor;
    ctx.fillText("WELCOME", 0.5 * canvas.width - (font[1] / 2),
                 0.09 * canvas.height);
    // stroke the text
    ctx.lineWidth = 0.5;
    ctx.strokeStyle = "#000000";
    ctx.strokeText("WELCOME", 0.5 * canvas.width - (font[1] / 2),
                   0.09 * canvas.height);
    ...
    ctx.beginPath();
    // write from top to bottom "Z" "L" "7" "7" "7" on the right between the
    // right line and the right of the canvas
    ...
    ctx.lineWidth = 0.7;
    ctx.strokeText("Z", 0.97 * canvas.width - (font[1] / 2),
                   0.3 * canvas.height);
    ctx.strokeText("L", 0.97 * canvas.width - (font[1] / 2),
                   0.4 * canvas.height);
    ctx.strokeText("7", 0.97 * canvas.width - (font[1] / 2),
                   0.5 * canvas.height);
    ctx.strokeText("7", 0.97 * canvas.width - (font[1] / 2),
                   0.6 * canvas.height);
    ctx.strokeText("7", 0.97 * canvas.width - (font[1] / 2),
    ...
    ctx.fillStyle = guildColor;
    ctx.fillText(`#${member.guild.memberCount}`, 0.07 * canvas.width,
                 0.87 * canvas.height);
    ctx.closePath();
    ...
    ctx.beginPath();
    ctx.arc(0.25 * canvas.width, 0.5 * canvas.height, 0.12 * canvas.width, 0,
            Math.PI * 2, true);
    ctx.closePath();
    ctx.clip();
    ...
    ctx.drawImage(avatar, 0.12 * canvas.width, 0.28 * canvas.height,
                  0.25 * canvas.width, 0.25 * canvas.width);
    ...
    const attachment =
        new AttachmentBuilder(canvas.toBuffer(), 'welcome-image.png');

    client.channels
        .fetch(system)

        .then(
            channel => {channel.send({files : [ attachment ]}).then(r => {
              // if user is bot we don't want to send him the message
              if (member.user.bot)
                return;

              let rulesChannel = member.guild.channels.cache.find(
                  ch => ch.name.includes('règlement') ||
                        ch.name.includes('règles'));
              let globalMsg;
              ...
              if (member.guild.id === "717344084695580672") {
                member.createDM()
                    .then(channel => {
                      channel.send(
                          `Salut, bienvenue sur le discord  **${
                              member.guild.name}** 😉\n` +
                          '\n' +
                ...
                if (member.guild.id === "880491243807846450") {
                  globalMsg =
                      "Vous êtes sur le discord général d'IPSA, par des étudiants pour des étudiants."
                } else {
                  globalMsg =
                      "Si tu n\'es pas sur le discord global IPSA: https://discord.gg/M98V7hDRx5"
                }
                member.createDM()
                    .then(channel => {
                      channel.send(
                          `Salut, bienvenue sur le discord **${
                              member.guild.name}** 😉\n` +
                          '\n' +
                          `Pour avoir accès à l\'intégralité du serveur accepte le règlement ici ---> <#${
                              rulesChannel.id}> \n` +
                          `> ${globalMsg} \n ` +
                          '\n' +
                          '                                                 ●¸.•*¨Ƹ̵̡Ӝ̵̨̄Ʒ¨*•.¸●\n' +
